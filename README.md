...
- **regressors**: Lista de strings que especifica os regressores utilizados. Não é necessário passar este parâmetro se a função possuir apenas o parâmetro *x* ou *x1, x2, ...* como regressor, por exemplo, `def f(x, b1, b2)` ou `def f(x1, x2, b1, b2)`;
...
- **generate_neural_network**: Para gerar uma rede neural artificial genérica, recebe dois parâmetros: *struct*(lista com quantidade de inputs, neuronios em cada camada e output) e *activation*(funções de ativação usada em cada camada, como a primeira camada não conta temos uma lista de tamanho len(struct) - 1);
- **generate_linear_spline**: Para gerar um *spline* usando funções lineares separadas por limiares $l_i$, recebe um único parâmetro: *lines*(inteiro, quantidade de linhas).
...
### Exemplo rede neural com **generate_neural_network**
...
```python
modelo = Regression(generate_neural_network(struct = [2, 3, 3, 1], activation = ['relu', 'relu', 'sigmoid']))
...
```
FUNCTION: new_generate_neural_network_2_3_3_1
LOSS FUNCTION: least_squares
REGRESSORS: x_1, x_2
LENTH OUTPUT: 1 (UNIVARIATE)
PARAMS:
...
Ou ainda uma lista de valores a serem preditos:
...
Segundo exemplo reduzindo a dimensionalidade:
...
def f(x1, x2, x3, w1, w2, w3, w4, w5, w6, w7, w8, w9, w10, w11, w12, w13, w14, w15, w16, w17) -> (float, float, float):
  c11:float = s(x1*w1 + x2*w2 + x3*w3 + w16)
  c12:float = s(x1*w4 + x2*w5 + x3*w6 + w17)
  return [c11*w7 + c12*w8 + w13, c11*w9 + c12*w10 + w14, c11*w11 + c12*w12 + w15]
...
dados_1, dados_2, dados_3, dados_4, dados_5, dados_6, dados_7, dados_8 = [], [], [], [], [], [], [], []
n:int = 2
k:int = 10

for _ in range(k):
  temp = [random(), random(), random()]
  temp = [*temp, [temp[0] - 3, temp[1] - 3, temp[2] - 3]]
  dados_1.append(temp.copy())

for _ in range(k):
  temp = [random() + n, random(), random()]
  temp = [*temp, [temp[0] - 3, temp[1] - 3, temp[2] - 3]]
  dados_2.append(temp.copy())

for _ in range(k):
  temp = [random(), random() + n, random()]
  temp = [*temp, [temp[0] - 3, temp[1] - 3, temp[2] - 3]]
  dados_3.append(temp.copy())

for _ in range(k):
  temp = [random(), random(), random() + n]
  temp = [*temp, [temp[0] - 3, temp[1] - 3, temp[2] - 3]]
  dados_4.append(temp.copy())

for _ in range(k):
  temp = [random() + n, random() + n, random() + n]
  temp = [*temp, [temp[0] - 3, temp[1] - 3, temp[2] - 3]]
  dados_5.append(temp.copy())

for _ in range(k):
  temp = [random(), random() + n, random() + n]
  temp = [*temp, [temp[0] - 3, temp[1] - 3, temp[2] - 3]]
  dados_6.append(temp.copy())

for _ in range(k):
  temp = [random() + n, random(), random() + n]
  temp = [*temp, [temp[0] - 3, temp[1] - 3, temp[2] - 3]]
  dados_7.append(temp.copy())

for _ in range(k):
  temp = [random() + n, random() + n, random()]
  temp = [*temp, [temp[0] - 3, temp[1] - 3, temp[2] - 3]]
  dados_8.append(temp.copy())

dados = [*dados_1, *dados_2, *dados_3, *dados_4, *dados_5, *dados_6, *dados_7, *dados_8]
colors = [*["blue"] * k, *["red"] * k, *["green"] * k, *["grey"] * k, *["pink"] * k, *["yellow"] * k, *["black"] * k, *["purple"] * k]
...
```
FUNCTION: f
LOSS FUNCTION(least_squares_multivariate): 1.02027349
REGRESSORS: x1, x2, x3
LENTH OUTPUT: 3 (MULTIVARIATE)
PARAMS:
  w1 = -0.21856788 (w: 0.93305773)
  w2 = -0.70089477 (w: 0.92572803)
  w3 = 1.04726199 (w: 0.93224252)
  w4 = 0.19325063 (w: 0.93305773)
  w5 = -0.48449476 (w: 0.92572803)
  w6 = -0.01427655 (w: 0.93224252)
  w7 = -0.31313602 (w: 0.86133541)
  w8 = 1.03627477 (w: 0.86133541)
  w9 = -0.14774773 (w: 0.87091987)
  w10 = -1.62078327 (w: 0.87091987)
  w11 = 0.79111506 (w: 0.86243128)
  w12 = -0.87125258 (w: 0.86243128)
  w13 = -2.36440081 (w: 0.63442108)
  w14 = -0.63719914 (w: 0.64253031)
  w15 = -0.39747654 (w: 0.63532989)
  w16 = -0.93143102 (w: 1.00000000)
  w17 = 1.05927996 (w: 1.00000000)
...
![EX_red_5](free_regression/imagens_testes/red_dim_5.png)

![EX_red_6](free_regression/imagens_testes/red_dim_6.png)

![EX_red_7](free_regression/imagens_testes/red_dim_7.png)
...
data = [0, 1]
for _ in range(8):
  data.append(-0.8*data[-1] + 1.2*data[-2])
...
modelo = Regression(f, print = True)
modelo.set_seed(1)
modelo.lock(d = data[-1][1]/data[-2][1])

modelo.run(data, save_infos = True)
print(modelo)

import matplotlib.pyplot as plt
loss = modelo.return_loss()

plt.figure(figsize = (10, 4))
plt.plot(loss["iteration"], loss["loss"], alpha = 0.4, zorder = 0, label = "Loss")
plt.plot(loss["iteration"], loss["loss"].rolling(window = 300).quantile(0.2), alpha = .7, zorder = 1, label = "Loss Smoothed")
plt.plot(loss["iteration"], loss["best_loss"], color = "red", linestyle = "--", zorder = 1, label = "Min Loss")
plt.yscale("log")
plt.xlabel("Iteration")
plt.ylabel(r"Loss ($log_{10}$)")
plt.title("Loss")
plt.legend()
plt.show()
```

![EX_loss](free_regression/imagens_testes/image_loss.png)